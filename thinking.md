...
Row is inserted with reply_status = 'drafted' and final_reply = NULL.
If an agent edits before sending: final_reply is populated, reply_status = 'edited'.
On delivery: reply_status becomes auto_sent, agent_sent, or escalated; sent_at is stamped.
...
Classify the message as a high-priority complaint and set action = escalate.
Notify the on-call operations team, villa caretaker, and property manager via WhatsApp/SMS/email.
Create a conversations record with the booking reference, property ID, and full message history.
Start a 30-minute SLA timer for human acknowledgement.
